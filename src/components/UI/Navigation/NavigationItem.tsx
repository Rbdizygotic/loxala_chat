...
const NavigationItem: React.FC<{ item: NavigationItemType }> = ({ item }) => {
    return <div className={`${item.notifcation > 0 ? "bg-[#F6F3FC]" : "bg-white"} justify-between flex px-[8px] py-[10px] rounded-[8px]`}>
        <div className="flex items-center gap-2 self-stretch">
            <img src={item.img} alt={item.name} />
            <span className="font-openSans text-[14px] font-normal leading-normal text-[#282529] ">{item.name}</span>
        </div>
        {item.notifcation > 0 &&
